...
decibel.
  ...
  affect, the supervision delivers a measurable gain at both compression rates and
reduces level error, not only spectral error.
...
tax}. Real encoders depart from affine, so the latent recovery is exact only
in that limit.
...
encode-then-decode path). SI-SDR$_\mathrm{lin}$ shares the decode-vs-decode structure of prior
additivity metrics \cite{torres2026linearity} and can move without ground-truth equivariance moving (\secref{sec:mixing}), so SI-SDR$_\mathrm{lin}^\mathrm{gt}$ and MixRate are our metrics of record. SI-SDR
fits a gain before scoring, so we report that gain separately, on 3{,}000
samples. FAD uses music-finetuned LAION-CLAP embeddings of non-overlapping
10\,s windows at 48\,kHz, not VGGish, so values are not comparable with
VGGish FAD.
...
its stems sum to the mixture at only $18.5$\,dB), giving 5{,}880 (track,
stem, chunk) units; chunk selection is a pure function of the seed and the
sorted track list, so all models see identical units and every contrast is
paired. Confidence intervals come from a paired cluster bootstrap over recordings
($10^4$ resamples), the correlated unit: 49 tracks on the test set and 240
on the held-out corpus. Mixing pairs join two chunks, about $80\%$ from the same recording, so those
intervals weight a pair by the multiplicity of each member. Intervals are a \emph{conditional
...
is active ($24{,}648$ of $28{,}800$); this excludes silent \emph{targets}
...
$+1.2$ to $+5.2$ at $15.3\times$ (\tabref{tab:subtraction}).
...
selected on the split we report, we repeat the intervention on MoisesDB
\cite{pereira2023moisesdb}, 240 recordings absent from training and from
selection, under a protocol registered before the audio was downloaded, with
frozen weights and verified hashes. Raw subtraction again favours the mixing models by $+2.3$ to $+4.5$\,dB and
recentering removes $87$--$95$\% of that, shrinking the gap on $229$, $231$
and $236$ of the $240$ recordings (\tabref{tab:origin}); on the rest it overshoots and reverses the sign. The residue is resolvable here, $+0.29$, $+0.12$ and $+0.42$\,dB with
intervals excluding zero, where the $49$-track set could not. The three contrasts are $\Ldec$ and $\Ldec{+}$disc against no mixing at
...
directions, $+0.35$\,dB on bass against $-0.34$ on vocals at $15.3\times$.
...
$+0.9$\,dB; raw subtraction loses to it everywhere.
...
raising decode-vs-decode SI-SDR$_\mathrm{lin}$ from $12.1$ to $16.0$\,dB. The adversarial term increases decode-consistency without improving the
latent-interpolation output's ground-truth SI-SDR, so the two metrics rank
these configurations differently. We do not identify the mechanism, and we never score the mixed-input
reconstruction route against ground truth. Whether the discriminator acts alone is untested.

\textbf{Architecture.} The same mixed-latent supervision on Music2Latent
\cite{pasini2024music2latent}, under its native consistency loss, improves
ground-truth equivariance against a continued-training control
($-9.5\!\to\!-7.8$\,dB) while waveform agreement stays poor ($-11.2$\,dB
subtraction). The systems differ in too many respects to isolate a cause; we
...
cannot affect and which improves at both rates. Latent arithmetic should therefore be reported against a declared origin and
against the model's own reconstruction of the target.
...
training sets no explicit seed. Intervals quantify recording-level
evaluation variation conditional on these checkpoints, not variation across
training runs. Selection used the test split; recovery is exact only for an affine encoder;
