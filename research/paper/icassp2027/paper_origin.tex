...
decoders, changes stem-subtraction scores by several decibels. Applying it to five fixed checkpoints, with no retraining, removes most of
the apparent advantage of mixing supervision on subtraction: on a held-out
corpus of 240 recordings that took no part in training or in checkpoint
selection, an advantage of $+2.3$ to $+4.5$\,dB falls to $+0.1$ to
$+0.4$\,dB, on every one of the 240 tracks, and every model lands within
...
corpus: recentering removes $87$--$91$\% of the advantage that mixing
supervision appears to have on stem subtraction, on every track of both
corpora, leaving a residue below half a decibel.
...
offset norm alone does not determine the decoded error. \textbf{Held-out
replication, and a residue.} Because these checkpoints were selected on the
same split we report (\secref{sec:protocol-setup}), we repeat the whole
...
audio was downloaded: frozen weights with verified hashes, the mixture
defined as the sum of the four stems, and units dropped where the target
stem is inactive ($85.6$\% kept). Raw subtraction again favours the mixing
models by $+2.3$ to $+4.5$\,dB, and recentering again removes most of it, on
every one of the 240 tracks (\tabref{tab:origin}). The residue, however, is
resolvable here: $+0.29$, $+0.12$ and $+0.42$\,dB with intervals excluding
zero, where the $49$-track set could not separate it from zero. The
intervention accounts for $87$--$91$\% of the apparent advantage rather than
all of it, and the smaller corpus lacked the resolution to say so.
...
Mixing supervision moves where the arithmetic error sits across sources more
than it moves the mean. The residual gap to the reference is what decoding
leaves after recentering, not a direct measure of encoder non-affinity.
...
(each mixing unit involves two recordings), $\Ldec$ improves ground-truth
equivariance by $+1.73$\,dB $[+1.71,+1.83]$ at $7.66\times$ and the combined
recipe by $+1.33$ $[+1.28,+1.46]$ at $15.3\times$, and the adversarial term
\emph{costs} $0.49$\,dB $[-0.51,-0.47]$ against $\Ldec$ alone while raising
decode-vs-decode from $9.7$ to $13.8$\,dB. On the test set, $\Ldec$ improves
...
checkpoints, removes $87$--$91$\% of the advantage that mixing supervision
appeared to have there, on every track of both a $49$-track test set and a
pre-registered $240$-track held-out corpus, leaving a residue below half a
decibel. What the supervision buys is convex equivariance, which the intervention
