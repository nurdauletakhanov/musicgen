...
quantized codecs (DAC \cite{kumar2024dac}, EnCodec \cite{defossez2023encodec})
cannot represent interpolated codes at all. Torres et al.~\cite{torres2026linearity} induce the property in a
...
structure of prior additivity metrics \cite{torres2026linearity} and is
gameable (\secref{sec:mixing}); SI-SDR$_\mathrm{lin}^\mathrm{gt}$ and MixRate
are our metrics of record. SI-SDR fits a gain before scoring, so it does not
...
supervision raises subtraction quality from $+3.4$ to $+5.7$\,dB at
...
never removal in absolute terms. Autoencoding the mixture and doing nothing
scores $+2.2$\,dB ($+1.7$ at $15.3\times$); decoding both signals and
subtracting waveforms scores $+7.5$ ($+6.5$). Raw latent subtraction falls
below waveform subtraction for every configuration, recentered latent
subtraction beats it by $+0.7$ to $+0.9$\,dB (intervals exclude zero), with
or without the loss.
...
$7.66\times$, $\Ldec$ improves ground-truth equivariance from $8.0$ to
$10.2$\,dB and is the only configuration with MixRate $<1$; reconstruction
SI-SDR ($11.2$--$11.4$\,dB) and FAD ($0.044$--$0.046$) are flat, so the
property is bought without a measured quality cost at this rate. Trained from
...
raising decode-vs-decode SI-SDR$_\mathrm{lin}$ from $12.1$ to $16.0$\,dB. The
discriminator pulls the two decode paths onto a common realistic manifold
without moving either toward $\bar{x}$, which is direct evidence that
SI-SDR$_\mathrm{lin}$ is gameable. Whether the discriminator has an effect
...
under independent-noise decoding, the protocol implied by prior reports, and
$+3.6$ and $+5.9$\,dB under shared-noise decoding: a $13$\,dB swing driven
by the protocol rather than the model. We use shared decode noise
everywhere.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.80\linewidth]{figures/fig_protocol.pdf}
  ...
  negative scores reflect decoder noise, consistent with phase variance, not
  latent linearity.}
  \label{fig:protocol}
\end{figure}
...
tracks. What the supervision does buy is convex equivariance, which the
intervention provably cannot affect and which improves at both compression
rates. We therefore recommend reporting latent arithmetic against a declared
origin and against the model's own reconstruction of the target, and reading
raw subtraction rankings as statements about coordinates until that is done.
...
intervals are conditional on these checkpoints; exact recovery of the
residual latent holds only for an affine encoder; experiments are mono with
no listening test. Multi-seed estimates, stereo and latent-diffusion
