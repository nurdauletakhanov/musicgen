...
decoders, changes stem-subtraction scores by several decibels. Applying it to five fixed checkpoints, with no retraining, removes most of the apparent advantage of mixing supervision on subtraction.
On a held-out corpus of 240 recordings that took no part in training or in
checkpoint selection, it reduces that advantage by $87$--$95$\%, leaving a
residual $+0.12$ to $+0.42$\,dB, and every model lands in aggregate within
$1$\,dB of its own reconstruction of the residual. On convex mixing, which the same reparameterization provably cannot
...
on the held-out corpus. Mixing pairs join two recordings, and $80\%$ of them
fall within one recording, so those intervals resample recordings and weight
a pair by the multiplicity of each member. Intervals are a \emph{conditional
...
selection. On the held-out corpus a unit is scored only where the removed
stem is active ($24{,}648$ of $28{,}800$ kept; the rule excludes silent
...
$+6.0$\,dB for every evaluated configuration and leaves each within
$0.4$--$0.5$\,dB of its own reconstruction reference; the models that gain
most are the ones that looked worst. The between-model advantage falls to
...
the $49$-track set could not separate it from zero. The intervention
accounts for most of the apparent advantage, not all of it.
...
Mixing supervision moves where the arithmetic error sits more than it moves
the mean.
...
\textbf{Architecture.} The same objective on the Music2Latent
\cite{pasini2024music2latent} consistency autoencoder improves ground-truth
