...
model to within $0.5$\,dB of its reconstruction ceiling on all 49 test tracks.
...
\begin{enumerate}
  ...
  latent origin rather than the loss (\secref{sec:downstream}).
...
scores $+7.5$ ($+6.5$). Raw latent subtraction is below waveform subtraction
for every model, origin-corrected latent subtraction above it by $+0.7$ to
$+0.9$\,dB (track-cluster CIs exclude zero), with or without the loss.
...
silence removes for every model. On a consistency-model autoencoder the loss
...
identifying each factor's effect there but not its variation across training
runs; experiments are mono; and we conduct no listening test. Stereo,
multi-seed estimates, and latent-diffusion composition are future work.
