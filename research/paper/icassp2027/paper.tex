...
autoencoders do not learn this structure on their own, and prior work induces
it only indirectly, through data augmentation. We ask which training signal is
actually responsible for mixing-equivariance, and answer with a controlled
...
mixing-equivariant \emph{at no reconstruction cost}, and latent subtraction
...
makes the operation viable at all ($+1.2\!\to\!+5.2$\,dB). The interventions
...
(related to mixup \cite{zhang2018mixup}). This leaves open \emph{which}
training signal is responsible for the property, whether the encoder or the
decoder must carry it, and whether the metrics used to certify it measure it
at all. We answer these questions with a controlled study: an explicit loss
(\secref{sec:method}) that compares decoded latent interpolations to true
waveform mixes, whose effect we then isolate by intervening on one factor at a
...
\begin{enumerate}
  ...
  loss is the active ingredient; an adversarial term on the mixed path
  inflates a decode-consistency metric with no ground-truth gain, exposing the
  metric as a confounded proxy; and an encoder-only penalty or a frozen
  encoder both fail, showing the property must be enforced through the decoder
  with both networks adapting.
...
representation is characterized by how it transforms under a known group
action on the data \cite{higgins2018towards}; Verma and Berger
...
(25k-step fine-tunes from a shared converged baseline). Every row differs from
the baseline in exactly one factor, so each comparison isolates that factor's
effect. Three observations drive the rest of the paper.
...
same $5{,}880$ (track, stem, chunk) units, so we compare them pairwise and
bootstrap the mean difference ($10^4$ resamples). The gains sit far outside
evaluation variance: $\Ldec$ over the no-mixing control is $+2.33$\,dB
(95\% CI $[+2.26,+2.39]$, improving $77\%$ of units), and at $15.3\times$ the
matched-control gain is $+4.02$\,dB ($[+3.93,+4.11]$, $90\%$ of units).
Disc-on-mix beats $\Ldec$ alone by only $+0.31$\,dB ($[+0.29,+0.33]$) here
while scoring $0.36$\,dB \emph{lower} on ground-truth equivariance
(\tabref{tab:ablation}): the two differ little, and in opposite directions
depending on the metric. These intervals cover evaluation variance only, not
...
\textbf{Limitations.} All results use a single training seed. Our confidence
intervals cover evaluation variance, not variation across initializations, so
small differences (e.g.\ $\Ldec$ vs.\ $\Ldec{+}$disc, $0.31$\,dB) remain
