...
space mixing-equivariant at no measured reconstruction cost, at $2\times$
...
consistency-model autoencoder the loss improves latent geometry but not
waveform arithmetic. Latent stem subtraction, in contrast, is governed by the
...
configuration to within $0.5$\,dB of its reconstruction ceiling in aggregate,
...
borrow the framing and not the formalism. Verma and Berger
\cite{verma2021framework} build equivariance into the architecture; we treat
it as a property to be \emph{induced} by supervision and isolate which
supervision induces it.
...
$-1.8$\,dB), as our phase-coherence finding predicts.
...
shared noise but scores $-1.8$\,dB under independent noise, with no latent
operation at all, so the decoder's sampling alone accounts for most of the
collapse. \figref{fig:protocol}
...
\emph{worsens} FAD, so the gain is the loss, not domain adaptation. Both metric families agree on the improvement.
Absolute SDR$_\mathrm{lin}^\mathrm{gt}$ nevertheless stays negative: this
configuration improves latent \emph{geometry} (MixRate, $\ell_\mathrm{lat}$)
without reaching usable waveform arithmetic. The systems differ in encoder,
...
\textbf{The gain is not domain-driven.} \tabref{tab:persource} breaks
SDR$_\mathrm{lin}^\mathrm{gt}$ out by test domain (indie production, solo
piano, studio mixes) against the matched control at each rate. The improvement
appears on all three at both rates ($+2.0$ to $+3.1$\,dB at $7.66\times$;
$+1.2$ to $+3.2$ at $15.3\times$), so it is a property of the latent space, not
...
the \emph{linearity tax}, the shortfall against that encode--decode ceiling.
...
configuration, leaving each $0.37$--$0.53$\,dB from its ceiling in aggregate
...
$[-0.15,+0.11]$), the $15.3\times$ pair $+0.01$ ($[-0.19,+0.21]$). The raw advantage was the no-mixing models' larger latent
...
and~\ref{tab:persource}), which the correction leaves intact. The residual
$0.4$--$0.5$\,dB is the encoder's departure from affinity; absolute quality
stays bounded by reconstruction.

\textbf{Baselines without latent arithmetic.} The unchanged mixture scores
$+17.1$\,dB against the residual, above every autoencoder's ceiling, since the
...
on the mixed path, as the signal responsible for convex mixing-equivariance, at
no measured reconstruction cost and across compression rates. Raw stem
subtraction is instead governed by the latent offset: one encode of silence
recovers the residual's own latent and, for every evaluated GAN configuration,
the model's reconstruction of it. On a consistency-model autoencoder the loss
improves latent geometry but not waveform arithmetic.
\textbf{Limitations.} Comparisons are matched at a single initialization,
identifying each factor's effect there but not across runs; experiments are
mono with no listening test. Stereo, multi-seed estimates, and latent-diffusion
