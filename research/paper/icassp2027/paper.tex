...
latent origin, not the loss: adding the encoded silence $f(0)$ brings every
evaluated GAN configuration to within $0.5$\,dB of its reconstruction ceiling
in aggregate, removing the loss's mean subtraction advantage.
...
offset, which we treat empirically (\secref{sec:downstream}). Where it holds,
...
We also report the \emph{linearity tax}, the shortfall of $\hat{x}$ against the
model's own encode--decode ceiling on the residual.
...
and~\ref{tab:persource}), which the correction leaves intact.
...
at no measured reconstruction cost and across compression rates. Raw stem
subtraction is instead governed by the latent offset, which one encode of
silence removes for every evaluated GAN configuration. On a consistency-model autoencoder the loss
improves latent geometry but not waveform arithmetic, so practical claims are
confined to the GAN autoencoder we test.
\textbf{Limitations.} Comparisons are matched at a single initialization, so
they identify each factor's effect there but not its variation across runs;
experiments are mono and we conduct no listening test. Stereo, multi-seed
